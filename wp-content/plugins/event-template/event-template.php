...
//  // Ensure we are in wp-admin before performing any additional actions
 ...
 function ci_event_init($post) {
   ...
   if( class_exists('TribeEventsAPI')) {
     ...
     jQuery(function(){
       var eventform = '<form class="ci-event-template" method="post" action=""><input name="post_title" type="text" value="" placeholder="Event Name" /><br /><input name="post_venue" type="text" value="" placeholder="Location" /><br /><input name="start_date" type="text" value="" placeholder="Start Date (mm/dd/yyyy)" /><input name="end_date" type="text" value="" placeholder="End Date (mm/dd/yyyy)" /><br /><input name="small_locker" type="checkbox" id="small_locker" value=""/><label for="small_locker">Small Locker</label><br /><input name="medium_locker" type="checkbox" id="medium_locker" value=""/><label for="medium_locker">Medium Locker</label><br /><input name="large_locker" type="checkbox" id="Large_locker" value=""/><label for="large_locker">large Locker</label><br /><input type="hidden" name="new_event" value="" /><input type="submit" name="submit" value="Create Event" /></form>';

        jQuery("body.events-cal #wpbody-content a.page-title-action").replaceWith(eventform);
        jQuery("body.forms_page_gf_entries #post-body-content").append(eventform);
     });
     </script>
     <?php
   }
